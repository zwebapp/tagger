...
class freetag {
	 ...
	 */ 
	protected $_db_user = 'changeme';
	protected $_db_pass = 'changeme';
	protected $_db_host = 'localhost';
	protected $_db_name = 'freetag'; 
	/**#@-*/
	...
	function get_objects_with_tag($tag, $offset = 0, $limit = 100, $tagger_id = NULL) {
		...
		$sql = "SELECT DISTINCT object_id
			FROM ${prefix}t_tagger_tagged_objects INNER JOIN ${prefix}t_tagger_tags ON (tag_id = id)
			WHERE tag = $tag
			$tagger_sql
	 ...
	 */
	 function get_objects_with_tag_combo($tagArray, $offset = 0, $limit = 100, $tagger_id = NULL) {
		...
		$sql = "SELECT ${prefix}t_tagger_tagged_objects.object_id, tag, COUNT(DISTINCT tag) AS uniques
			FROM ${prefix}t_tagger_tagged_objects 
			INNER JOIN ${prefix}t_tagger_tags ON (${prefix}t_tagger_tagged_objects.tag_id = ${prefix}t_tagger_tags.id)
			WHERE ${prefix}t_tagger_tags.tag IN ($tag_sql)
			$tagger_sql
			GROUP BY ${prefix}t_tagger_tagged_objects.object_id
	...
	function safe_tag($tagger_id, $object_id, $tag) {
		...
		if (!$rs->EOF) {
			...
			// Add new tag! 
			$sql = "INSERT INTO ${prefix}t_tagger_tags (tag, raw_tag) VALUES ($normalized_tag, $tag)";
			$rs = $db->Execute($sql) or die("Syntax Error: $sql");
			$tag_id = $db->Insert_ID();
		}
		if (!($tag_id > 0)) {
		...
		$sql = "INSERT INTO ${prefix}t_tagger_tagged_objects
	...
	function delete_object_tag($tagger_id, $object_id, $tag) {
		...
		if ($tag_id > 0) {
			...
			$sql = "DELETE FROM ${prefix}t_tagger_tagged_objects
	...
	function get_tag_id($tag) {
		...
		$sql = "SELECT id FROM ${prefix}t_tagger_tags
			WHERE 
			tag = $tag
			LIMIT 1
			";	
	...
	function get_most_popular_tags($tagger_id = NULL, $offset = 0, $limit = 25) {
		...
		$sql = "SELECT tag, COUNT(*) as count
			FROM ${prefix}t_tagger_tags INNER JOIN ${prefix}t_tagger_tagged_objects ON (id = tag_id)
			WHERE 1
			$tagger_sql
			GROUP BY tag
	...
	function get_most_recent_objects($tagger_id = NULL, $tag = NULL, $offset = 0, $limit = 25) {
		...
		if (!$tag) {
			$sql = "SELECT DISTINCT object_id, tagged_on FROM
				${prefix}t_tagger_tagged_objects
				WHERE 1
				$tagger_sql
	...
	function get_tag_cloud_tags($max = 100, $tagger_id = NULL, $offset = 0) {
		...
		$sql = "SELECT tag, COUNT(object_id) AS quantity
			FROM ${prefix}t_tagger_tags INNER JOIN ${prefix}t_tagger_tagged_objects
			ON (${prefix}t_tagger_tags.id = tag_id)
			WHERE 1
			$tagger_sql
			GROUP BY tag
	...
	function similar_tags($tag, $max = 100, $tagger_id = NULL) {
		...
		$sql = "SELECT t1.tag, COUNT( o1.object_id ) AS quantity
			FROM ${prefix}t_tagger_tagged_objects o1
			INNER JOIN ${prefix}t_tagger_tags t1 ON ( t1.id = o1.tag_id )
			INNER JOIN ${prefix}t_tagger_tagged_objects o2 ON ( o1.object_id = o2.object_id )
			INNER JOIN ${prefix}t_tagger_tags t2 ON ( t2.id = o2.tag_id )
	...
	function similar_objects($object_id, $threshold = 1, $max_objects = 5, $tagger_id = NULL) {
		...
		$sql = "SELECT matches.object_id, COUNT( matches.object_id ) AS num_common_tags
			FROM ${prefix}t_tagger_tagged_objects as matches
			INNER JOIN ${prefix}t_tagger_tags as tags ON ( tags.id = matches.tag_id )
